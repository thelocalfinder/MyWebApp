...
namespace MyWebApp.Controllers
{
    ...
    public class ExportController : ControllerBase
    {
            ...
            [FromQuery] DateTime? endDate = null)
        {
            try
            {
                _logger.LogInformation("Starting brands export");
                ...
                {
                    query = query.Where(b => b.Products
                        .Any(p => (!startDate.HasValue || p.AddedAt >= startDate) &&
                                 (!endDate.HasValue || p.AddedAt <= endDate)));
                }
                ...
                var brands = await query
                    .Select(b => new
                    {
                        ...
                        AverageClicks = b.Products.Any() ? b.Products.Average(p => p.ClickCount) : 0,
                        TotalRevenue = b.Products.Sum(p => p.DiscountedPrice ?? p.Price)
                    })
                ...
                if (brands.Count == 0)
                {
                    ...
                    return NotFound("No brands found matching the specified criteria");
                }

                var csv = new StringBuilder();
                csv.AppendLine("ID,Name,Website,ProductCount,TotalClicks,AverageClicks,TotalRevenue");

                foreach (var brand in brands)
                {
                    csv.AppendLine($"{brand.ID},{EscapeCsvField(brand.Name)},{EscapeCsvField(brand.WebsiteURL)},{brand.ProductCount},{brand.TotalClicks},{brand.AverageClicks:F2},{brand.TotalRevenue:F2}");
                }

                _logger.LogInformation("Brands export completed successfully");
                return File(Encoding.UTF8.GetBytes(csv.ToString()), "text/csv", "brands.csv");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error exporting brands: {Message}", ex.Message);
                return StatusCode(500, $"An error occurred while exporting brands: {ex.Message}");
            }
        }
